...
def chevereto_api_upload(imgpath: str, url: str, api_key: str):
    ...
    try:
        ...
        logger.error("json decoder failed")
        res = {}
    except Exception as r:
    ...
    for imgpath in imgpaths:
        ...
        while success == 0 and trynum < 5:
            ...
            try:
                ...
                success = 0
                logger.warning(domain + '第' + str(imgnum) + '张图片' + imgpath + '第' +
                               str(trynum) + '次上传失败，正在重试...原因: %s' % (r))
            if imgstr is None:
                success = 0
                logger.warning(
                    domain + '第' + str(imgnum) + '张图片' + imgpath + '第' + str(trynum) + '次上传失败，正在重试...')
        if success == 1:
            if form.lower() == 'img':
    ...
    for imgpath in imgpaths:
        ...
        while success == 0 and trynum < 5:
            ...
            try:
                ...
                success = 0
                logger.warning(domain + '第' + str(imgnum) + '张图片' + imgpath + '第' + str(
                    trynum) + '次上传失败，正在重试...原因: %s' % (r))
            if imgstr == None:
                success = 0
                logger.warning(
                    domain + '第' + str(imgnum) + '张图片' + imgpath + '第' + str(trynum) + '次上传失败，正在重试...')
