...
from auto_upload.utils.para_ctrl.readargs import read_args
...
def read_para():
    ...
    if not args.img_upload + args.sign + args.upload + args.douban_info + args.media_img == 1:
        logger.error(
            '参数输入错误，上传模式 -u,签到模式 -s,上传图床模式 -iu,获取豆瓣信息 -di, 获取视频截图链接 -mi, 必须且只能选择一个。')
        raise ValueError(
            '参数输入错误，上传模式 -u,签到模式 -s,上传图床模式 -iu,获取豆瓣信息 -di, 获取视频截图链接 -mi, 必须且只能选择一个。')
    ...
    if 'basic' in au_data and 'cookies_path' in au_data['basic']:
        for item in au_data['site info']:
            if not 'cookiefile' in au_data['site info'][item]:
                au_data['site info'][item]['cookiefile'] = os.path.join(au_data['basic']['cookies_path'],
                                                                        'cookie_' + item + '.json')
    ...
    au_data[
        'mod'] = args.media_img * 'media_img' + args.img_upload * 'img_upload' + args.sign * 'sign' + args.upload * 'upload' + args.douban_info * 'douban_info'
    ...
    if args.upload:
        ...
        for item in au_data['path info']:
            if not 'path' in au_data['path info'][item] or au_data['path info'][item]['path'] == None or \
                    au_data['path info'][item]['path'] == '':
                logger.error('参数输入错误，' + item + '请至少输入一个本地文件地址')
                raise ValueError('参数输入错误，' + item + '请至少输入一个本地文件地址')
            if 'type' in au_data['path info'][item] and not (
                    'anime' in au_data['path info'][item]['type'].lower() or 'tv' in au_data['path info'][item][
                'type'].lower() or 'movie' in au_data['path info'][item]['type'].lower()):
        ...
        if not 'start' in au_data['qbinfo'] or not (
                int(au_data['qbinfo']['start']) == 1 or int(au_data['qbinfo']['start']) == 0):
            au_data['qbinfo']['start'] = 0
            logger.warning('未找到qbinfo中的start(添加到qb的种子是否自动开始)参数,已设置为0(不自动开始)')
        if not 'headless' in au_data['basic'] or not (
                int(au_data['basic']['headless']) == 1 or int(au_data['basic']['headless']) == 0):
    ...
    if args.media_img:
        ...
        if not 'basic' in au_data or not 'picture_num' in au_data['basic'] or (
                au_data['basic']['picture_num'] == None or au_data['basic']['picture_num'] == ''):
            logger.warning('未找到yaml文件中截图数量参数picture_num,已设置为3')
            if not 'basic' in au_data:
                au_data['basic'] = dict()
            au_data['basic']['picture_num'] = 3

        if not 'basic' in au_data or not 'screenshot_path' in au_data['basic'] or (
                au_data['basic']['screenshot_path'] == None or au_data['basic']['screenshot_path'] == ''):
